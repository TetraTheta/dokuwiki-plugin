...
jQuery(function () {
  ...
  jQuery('.dokuwiki .folder').click(function folded_toggle(evt) {
    ...
    if ($id.hasClass('hidden')) {
      ...
      jQuery(this).addClass('open').attr('title', folded_hide);
    } else {
      $id.addClass('hidden').removeClass('open');
      jQuery(this).removeClass('open').attr('title', folded_reveal);
    }
...
const foldedStyle = document.createElement('style');
foldedStyle.type = 'text/css';
foldedStyle.media = 'screen';
foldedStyle.appendChild(
  document.createTextNode('.folded.hidden { display: none; } .folder .indicator { visibility: visible; }')
);
document.head.appendChild(foldedStyle);
