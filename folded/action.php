...
class action_plugin_folded extends DokuWiki_Action_Plugin {
  ...
  function register(Doku_Event_Handler $controller) {
    ...
    $controller->register_hook('MENU_ITEMS_ASSEMBLY', 'AFTER', $this, 'add_button_new', []);
  }
    ...
    if ($this->getConf('show_fold_unfold_all_button') && $event->data['view'] == 'main') {
      ...
      $event->data['items'] =
        array_slice($event->data['items'], 0, -1, true) + [
          'fold_unfold_all' =>
            '<li>' .
            '<a href="javascript:void(0);" class="fold_unfold_all" onclick="fold_unfold_all();" rel="nofollow" title="' .
